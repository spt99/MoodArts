...
public interface ProductRepository extends JpaRepository<Product, Long> {
	...
	@Query("SELECT p From Product p where LOWER(p.title) Like %:query% OR LOWER(p.description) Like %:query% OR LOWER(p.Artist) LIKE %:query% OR LOWER(p.category.name) LIKE %:query%")
